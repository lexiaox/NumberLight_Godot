# ObstacleManager.gd — 障碍物碰撞自动生成器
extends Node

@export var collision_scale: float = 1.0

const OBSTACLE_PROFILES = {
    "Tree_":      {"w": 0.40, "h": 0.25, "y": 0.35},
    "Fence_":     {"w": 1.05, "h": 1.10, "y": 0.00},
...
func _ready():
    ...
    var count = 0
    scan_and_add_collision(scene, count)

func scan_and_add_collision(node: Node, count: int):
    for child in node.get_children():
        if child is CanvasLayer: continue
        if child is Sprite2D:
            if try_add_collision(child): count += 1
        scan_and_add_collision(child, count)

func get_content_rect(tex: Texture2D) -> Rect2i:
    var tex_size = tex.get_size()
    var key = "%dx%d" % [int(tex_size.x), int(tex_size.y)]
    if KNOWN_CONTENT_RECTS.has(key): return KNOWN_CONTENT_RECTS[key]
    var img = tex.get_image()
    if img:
        var rect = img.get_used_rect()
        if rect.size.x > 0 and rect.size.y > 0: return rect
    return Rect2i(0, 0, int(tex_size.x), int(tex_size.y))

func try_add_collision(sprite: Sprite2D) -> bool:
    for child in sprite.get_children():
        if child is StaticBody2D: return false
    ...
    var sx = sprite.scale.x; var sy = sprite.scale.y
    var off_x = cx_off * sx; var off_y = cy_off * sy
