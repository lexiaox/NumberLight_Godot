...
func _ready() -> void:
	...
	_junction_node = get_node_or_null("Decorations/JunctionNode")
...
func _process(_delta: float) -> void:
	...
	if is_near(_junction_node, foot, 136.0):
		inspect_junction_node()
		return
...
func inspect_junction_node() -> void:
	if GameState.investigated_protocol_junction:
		show_toast("协议汇流节点已经记录完成。", false)
		return
	start_dialogue("协议汇流节点", [
		"这里的协议汇流节点正在持续接收异常回响，深层链路已经不只是被擦过，而是被真正接入了。",
		"节点周围的能量回路仍然保留着旧设施的响应痕迹，说明异常还在尝试接管主交换位。",
		"先把这条线索带回农场，向父亲汇报当前进展。"
	], func():
		GameState.investigated_protocol_junction = true
		GameState.pending_protocol_junction_report = true
		update_labels()
		show_toast("已记录协议汇流节点线索。", true)
	)

func return_to_farm() -> void:
	if not GameState.investigated_protocol_junction:
		show_toast("先调查协议汇流节点，再返回农场。", false)
		return
	GameState.queue_farm_return("anomaly_return", "你已从协议汇流节点带回新的异常线索。")
