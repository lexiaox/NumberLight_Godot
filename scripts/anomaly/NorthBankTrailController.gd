...
func _ready() -> void:
	...
	_sluice_point = get_node_or_null("Decorations/SluiceAnchor")
...
func _process(_delta: float) -> void:
	...
	if is_near(_sluice_point, foot, 124.0):
		inspect_sluice()
		return
...
func inspect_sluice() -> void:
	if GameState.investigated_northbank_sluice:
		show_toast("北岸导流槽锚点已经记录完成。", false)
		return
	start_dialogue("北岸导流槽", [
		"导流槽锚点位置留着一圈明显的拆卸痕迹，说明这里曾被人为强行打开过。",
		"能量残痕继续沿着北岸向更深处延伸，下一段调查很可能会转向主井外环。",
		"先把这条线索带回农场，和父亲汇报当前进展。"
	], func():
		GameState.investigated_northbank_sluice = true
		GameState.pending_anomaly_report = true
		update_labels()
		show_toast("已记录北岸导流槽线索。", true)
	)

func return_to_farm() -> void:
	if not GameState.investigated_northbank_sluice:
		show_toast("先调查北岸导流槽锚点，再返回农场。", false)
		return
	GameState.queue_farm_return("anomaly_return", "你已从北岸导流槽带回新的异常线索。")
